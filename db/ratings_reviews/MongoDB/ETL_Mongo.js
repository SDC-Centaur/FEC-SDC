...
async function ETL(database = 'SDC', batchSize = 5000) {
  ...
  mainLoop: while (true) {
    ...
    const metaDocument = {
      ...
      characteristics: new Map(charKeys.map(key => [key, 0]))
    };
    ...
    while (true) {
      ...
      document.characteristics = createDocument(charKeys, charValues);
      ...
      for (const char in document.characteristics) {
        const value = metaDocument.characteristics.get(char);
        if (value === undefined) {
          metaDocument.characteristics.set(char, document.characteristics[char]);
        } else {
          metaDocument.characteristics.set(char, document.characteristics[char] + value);
        }
      }
